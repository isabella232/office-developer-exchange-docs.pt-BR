...
description: O elemento ReplyToItem contém uma resposta ao remetente de um item no Exchange store.
...
O **elemento ReplyToItem** contém uma resposta ao remetente de um item no Exchange store. 
...
|[ToRecipients](torecipients.md) <br/> |Contém uma matriz de destinatários de um item. Esses são os principais destinatários de um item.  <br/> |
