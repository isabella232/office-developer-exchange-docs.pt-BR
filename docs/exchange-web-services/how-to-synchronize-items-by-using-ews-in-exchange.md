...
Descubra como usar a API gerenciada de EWS ou o EWS para obter uma lista de todos os itens em uma pasta ou uma lista das alterações que tenham ocorrido em uma pasta, para sincronizar seu cliente.
  
EWS no Exchange usa a sincronização de item e a sincronização de pasta para sincronizar o conteúdo de caixa de correio entre o cliente e servidor. Sincronização de item obtém a lista inicial de itens em uma pasta e em seguida, ao longo do tempo, obtém as alterações que foram feitas para os itens e obtém novos itens também.
  
Observe que antes de você pode sincronizar itens em um cliente, primeiro você deve [sincronizar a hierarquia de pastas](how-to-synchronize-folders-by-using-ews-in-exchange.md). Após a pasta hierarquia é in-loco no cliente, se você estiver realizando a sincronização de item usando a API gerenciada de EWS, você primeiro [obter a lista inicial de itens na caixa de entrada](#bk_cesyncongoingewsma) usando o método [ExchangeService.SyncFolderItems](http://msdn.microsoft.com/en-us/library/microsoft.exchange.webservices.data.exchangeservice.syncfolderitems%28v=exchg.80%29.aspx) . Você, em seguida, atualize o valor do parâmetro _cSyncState_ durante as chamadas subsequentes para obter a lista de itens alterados na caixa de entrada. 
  
Para executar a sincronização de item usando o EWS, após [a hierarquia de pastas de sincronização](how-to-synchronize-folders-by-using-ews-in-exchange.md), você solicitar a [lista inicial de itens na caixa de entrada](#bk_ewsexamplea) usando a [operação SyncFolderItems](http://msdn.microsoft.com/library/7f0de089-8876-47ec-a871-df118ceae75d%28Office.15%29.aspx), analisar a resposta e, em seguida, em algum momento no futuro [ fazer as alterações aos itens na caixa de correio](#bk_ewsexamplec)e analisar a resposta. Depois que o cliente recebe a lista de itens iniciais ou alterados, ele [faz com que as atualizações localmente](#bk_nextsteps). Como e quando você recupera alterações no futuro depende do [padrão de design de sincronização](mailbox-synchronization-and-ews-in-exchange.md#bk_syncpatterns) de que seu aplicativo está usando. 
  
## <a name="get-the-list-of-all-items-or-changed-items-by-using-the-ews-managed-api"></a>Obter a lista de todos os itens ou itens alterados usando a API gerenciada de EWS
<a name="bk_cesyncongoingewsma"> </a>

O exemplo de código a seguir mostra como obter uma lista inicial de todos os itens na pasta caixa de entrada e, em seguida, obtenha uma lista de alterações nos itens na pasta caixa de entrada que tenham ocorrido desde a sincronização anterior. Durante a chamada inicial para o método [SyncFolderItems](http://msdn.microsoft.com/en-us/library/microsoft.exchange.webservices.data.exchangeservice.syncfolderitems%28v=exchg.80%29.aspx) , defina o valor de _cSyncState_ como nulo. Quando o método for concluído, salve o valor de _cSyncState_ localmente para usar na chamada do método **SyncFolderItems** próxima. Na chamada inicial e as chamadas subsequentes, os itens serão recuperados em lotes de dez, usando sucessivas chamadas ao método **SyncFolderItems** , até que nenhuma alteração mais permaneça. 
  
Este exemplo define o parâmetro _propertySet_ para IdOnly para reduzir chamadas para o banco de dados do Exchange, que é uma [prática recomendada de sincronização](mailbox-synchronization-and-ews-in-exchange.md#bk_bestpractices). Neste exemplo, assumimos que o **serviço** é um vínculo de objeto **ExchangeService** válido e que _cSyncState_ representa o estado de sincronização foi retornado por uma chamada anterior ao **SyncFolderItems**. 
...
Depois que você recuperar a lista de novos itens no servidor, [criar os itens no cliente](#bk_nextsteps).
...
A operação de **SyncFolderItems** é semelhante ao método [FindItems](http://msdn.microsoft.com/en-us/library/microsoft.exchange.webservices.data.exchangeservice.finditems%28v=exchg.80%29.aspx) em que ele não pode retorná elementos como os elementos do [corpo](http://msdn.microsoft.com/library/7851ea9b-9f87-4adc-a26f-7a27df4a9bca%28Office.15%29.aspx) ou [anexos](http://msdn.microsoft.com/library/b470e614-34bb-44f0-8790-7ddbdcbbd29d%28Office.15%29.aspx) . Se você precisar de propriedades que não podem ser retornadas pela operação de **SyncFolderItems** , defina o valor do elemento [BaseShape](http://msdn.microsoft.com/library/42c04f3b-abaa-4197-a3d6-d21677ffb1c0%28Office.15%29.aspx) como IdOnly quando você chamar **SyncFolderItems**e, em seguida, use a [operação GetItem](http://msdn.microsoft.com/library/e3590b8b-c2a7-4dad-a014-6360197b68e4%28Office.15%29.aspx) para obter as propriedades você requer para os itens que foram retornados pela operação de **SyncFolderItems** . 
  
Depois que você recuperar a lista de itens alterados no servidor, [atualizar os itens no cliente](#bk_nextsteps).
  
## <a name="update-the-client"></a>Atualizar o cliente
<a name="bk_nextsteps"> </a>

Se você estiver usando a API gerenciada de EWS, depois de obter a lista de itens novos ou alterados, use o método [LoadPropertiesForItems](http://msdn.microsoft.com/en-us/library/microsoft.exchange.webservices.data.exchangeservice.loadpropertiesforitems%28v=exchg.80%29.aspx) para obter propriedades nos itens novos ou alterados, compare as propriedades com os valores de locais e atualizar os itens no cliente. 
  
Se você estiver usando o EWS, use a [operação GetItem](http://msdn.microsoft.com/library/e3590b8b-c2a7-4dad-a014-6360197b68e4%28Office.15%29.aspx) obter propriedades nos itens novos ou alterados e atualizar os itens no cliente. 
